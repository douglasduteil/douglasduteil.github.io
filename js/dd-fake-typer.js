...
window.angular
  ...
  .controller('ddFakeTyperController', function FakeTyperCtrl ($scope, $interval, $q, $timeout, ddFakeTyperConfig) {
    var ctrl = this
    var tabs = ctrl.options = $scope.options = []
  ...
  return {
    ...
    link: function ($scope, $element, $attrs, ddFakeTyperController) {
      configKeys.forEach(function (configKey) {
        ddFakeTyperController[configKey] = window.angular.isDefined($attrs[configKey]) ? $scope.$parent.$eval($attrs[configKey]) : controllerDefaultConfig[configKey]
      })

      //
      if (window.angular.isDefined($attrs.order)) {
        var order = $scope.$parent.$eval($attrs.order)
        if (window.angular.isString(order)) {
          switch (order) {
            case 'random':
